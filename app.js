(function(){
  ...
  function renderSavingsChart(s){const pts=[];for(let y=0;y<=s.years;y++)pts.push({y,goal:annualGrowth(s.today,s.inflation,y),plan:y===0?s.existing:planAt(s,y)});const max=niceMax(Math.max(...pts.flatMap(p=>[p.goal,p.plan]))*1.08),g=chartBase(els.savingsChart,max,s.years),p1=pts.map((p,i)=>`${i?'L':'M'}${g.x(p.y)},${g.yy(p.goal)}`).join(' '),p2=pts.map((p,i)=>`${i?'L':'M'}${g.x(p.y)},${g.yy(p.plan)}`).join(' ');els.savingsChart.innerHTML+=`<path class="goal-line" d="${p1}"/><path class="plan-line" d="${p2}"/><circle class="dot-goal" cx="${g.x(s.years)}" cy="${g.yy(pts.at(-1).goal)}" r="5"/><circle class="dot-plan" cx="${g.x(s.years)}" cy="${g.yy(pts.at(-1).plan)}" r="5"/>`;}
